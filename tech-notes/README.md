...
| [0007](0007-pluggable-system-one-backends-and-laya-serve.md) | Pluggable System One Backends & Wire Compatibility with Laya HTTP Serving | Wire protocol parity between TypeSafe Jev `/v1/systemone` and `laya-serve` (PR #31), pluggable `SystemOneBackend` protocol architecture, auth header dynamics, and zero-downtime provider repointing. |
| [0008](0008-on-device-coreml-decision-engine.md) | On-Device Decision Models via Core ML, Apple Neural Engine, and Native Tokenization | Compiling ModernBERT (421M) and mmBERT (322M) to Core ML, ANE option marker gathering, 8-bit weight quantization, and zero-dependency Swift tokenization. |
